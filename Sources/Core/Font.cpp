...
U32 font_colors_presets[8] = { 0xff9f9faa, 0xff20e0ff, 0xff0000ff, 0xff4040ff, 0xff00ff00, 0xffffeb00, 0xff0066ff, 0xffffffff };
...
void Font_Add(char *font_name)
{
	...
	while (cur_exp != NULL)
	{
		...
		{
			if (strcicmp(cur_exp, "kernings") == 0)
			{
				parsing_group = FONT_PG_KERNINGS;
				continue;
			}
			if (strcicmp(cur_exp, "chars") == 0)
			{
	...
	if (succeed)
	{
		...
		TexManager_AddTexture(fonts[fonts_count].texture, fonts[fonts_count].smooth ? 0 : TEXTURE_FLAG_NEAREST);
...
void Font_PrintText(float x, float y, float text_scale, U32 text_color, char *text, U32 flags, char *font_name)
{
	...
	{
		if (strcicmp(fonts[i].name, font_name) == 0)
			current_font = i;
	}
