...
bool Vector2D_LineIntersection(Vector2D *a1, Vector2D *a2, Vector2D *b1, Vector2D *b2, Vector2D *result)
{
    Vector2D v1 =  Vector2D_Subtract(a2, a1);
	Vector2D v2 =  Vector2D_Subtract(b2, b1);

	float d = -v2.x * v1.y + v1.x * v2.y;

	if (d == 0.0f)
		return false;
	
    float u = (-v1.y * (a1->x - b1->x) + v1.x * (a1->y - b1->y)) / d;

	if (u >= 0.0f && u <= 1.0f)
	{
		float v = (v2.x * (a1->y - b1->y) - v2.y * (a1->x - b1->x)) / d;
